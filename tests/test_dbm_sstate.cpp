...
static shared_ptr<DBM_Symbolic_State> build_state(const std::vector<std::string> &locs, 
			   const Valuations  &dv,
...
TEST_CASE("Simple DBM model", "[DBM][backtrack]")
{
    Model::reset();
    ifstream ifs("dbm-test.forts");
    ...
    auto s_a = build_state({ "A" }, 
				     {}, 
				     "x<=2 & x >= 0 & x==y"); 
    auto s_b = build_state({ "B" }, 
				     {}, 
				     "x>=0 & y-x==2");
    auto s_c = build_state({ "C" }, 
				     {}, 
				     "x>=0 & y-x==0");
