...
def build_init_invariant(l, cpus, fl=True) :
    ...
    for i, e in enumerate(l):
      if inv != "" :
          inv = inv + " & "
      inv = inv + "p{0}<=T{0}".format(str(i+1))
            ...
            if fl :
                ...
                if c <= cpus :
                    ...
                    if 1 == 1:
                      if inv != "" :
                          inv = inv + " & "
                      inv = inv + "c{0}>=0 & p{0}<=D{0}".format(str(i+1))
                    else:
                      if inv != "" :
                          inv = inv + " & "
                      inv = inv + "c{0}>=0".format(str(i+1))
                    ...
                    if 1 == 1:
                      if inv != "" :
                          inv = inv + " & "
                      inv = inv + "p{0}<=D{0}".format(str(i+1))
            else:
                if c <= cpus :
                    #if i == len(l) - 1:
                    if i == i:
...
def gen_sched(out, ntasks, cpus) :    
    ...
    for i in range(1, ntasks+1) :
        id = str(i)
        out.write("    when p{0}>=T{0}  do {{p{0}'=0, c{0}'=C{0}}} goto x{0}R;\n".format(id))
    out.write("\n");
    ...
    for l in alist :
        ...
        for i,e in enumerate(act):
            ...
            out.write("    when c{0}>0 & p{0}==D{0} do {{}} goto error;\n".
                  format(str(e)))
        for i,e in enumerate(wlist):
            #if not e == len(l) : continue
            out.write("    when p{0}==D{0} do {{}} goto error;\n".
                  format(str(e)))
        ...
        for i,e in enumerate(inact):
            ...
            out.write("    when {0}p{1}>=T{1}   do {{p{1}'=0, c{1}'=C{1}}} goto {2};\n".
...
def main() : 
    ...
    parser.add_argument("cpus", help="The number of processors")     
