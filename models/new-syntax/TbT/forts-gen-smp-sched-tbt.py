    ...
    for l in alist :
        ...
        for i,e in enumerate(inact):
            ...
            if e == ntasks:
                continue
            l3 = l
            if count_active(l) == cpus-1:
                if not is_active(l, ntasks):
                    l3 = set_arrival(l, ntasks-1)
                    newstate = "x" + build_state(set_arrival(l3, e-1), cpus)
                    out.write("    when {0}p{1}-T{1}>=0   do {{p{1}'=0, c{1}'=C{1}, p{3}'=0,c{3}'=C{3}}}   goto {2};\n".
